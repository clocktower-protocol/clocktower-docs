...
En REST, pasa el mismo `?chainId=` en readiness, prepare y `POST /transactions/status` (`{ "txHash": "0x…" }`). Subscribe en MCP es solo Base; consulta `get_transaction_status` con `txHash`. Ver [Selección de cadena](/docs/developers/REST%20API/chain-selection).
