...
| **Wallet** | Firma transacciones; tu app las transmite a la cadena seleccionada (REST `?chainId=`; MCP siempre Base) |
...
4. **Transmitir** — tu app envía la(s) transacción(es) firmada(s) a la cadena en `unsignedTransactions[].chainId` (MCP: Base mainnet).
...
|------|------|----------|-----|
| 1. Preparación | `POST /check_subscribe_readiness_by_id` (**preferido**) o forma objeto | `check_subscribe_readiness_by_id` | Integrado en preflight de `subscribe()` |
| 2. Preparar | `POST /prepare/subscribe_by_id` (**preferido**) o forma objeto | `prepare_subscribe_by_id` | `subscribeWithApproval()` / `subscribeById` |
...
Las rutas by-id solo necesitan `from` + `id` de suscripción (amount/token/provider se cargan on-chain). La forma por objeto sigue disponible si ya tienes el objeto completo.

En REST, pasa el mismo `?chainId=` en readiness, prepare y `POST /transactions/status`. Subscribe en MCP es solo Base. Ver [Selección de cadena](/docs/developers/REST%20API/chain-selection).
