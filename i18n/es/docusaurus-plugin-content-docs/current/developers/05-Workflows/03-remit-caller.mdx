...
3. **Firmar y transmitir** — la wallet del caller firma y tu bot envía a la cadena en `unsignedTransactions[].chainId`.
...
|------|------|----------|-----|
| 1. Preparación | `POST /check_remit_readiness` | `check_remit_readiness` | `checkRemitReadiness()` (también corre dentro de `remit()` por defecto) |
| 2. Preparar | `POST /prepare/remit` | `prepare_remit` | `remit()` — escanea salvo `{ skipReadiness: true }` |
...
- En REST, pasa `?chainId=` para que readiness, prepare y status usen la misma cadena. En MCP, pasa el mismo `chainId` en esas tools (si se omite, se usa Base).

<IfFeature name="sdk">
SDK `remit()` lanza `RemitNotReadyError` y no envía una transacción cuando el precheck de la cola de vencimientos no encuentra trabajo. `runRemitUntilCaughtUp` escanea una vez por ronda y luego llama `remit({ skipReadiness: true })`.
</IfFeature>