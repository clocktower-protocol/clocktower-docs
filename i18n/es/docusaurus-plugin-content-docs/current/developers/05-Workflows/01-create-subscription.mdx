...
| **Wallet** | Firma y transmite a la cadena seleccionada (REST `?chainId=`; MCP `chainId` opcional, default Base) |
...
3. **Transmitir** — tu app la envía a la cadena en `unsignedTransactions[].chainId`.
...
En REST, pasa `?chainId=` en prepare y en el `GET /subscriptions/:id` posterior para quedarte en la misma cadena. En MCP, pasa el mismo `chainId` en prepare y `get_transaction_status` (si se omite, se usa Base). Ver [Selección de cadena REST](/docs/developers/REST%20API/chain-selection) y [Selección de cadena MCP](/docs/developers/MCP/mcp-chain-selection).