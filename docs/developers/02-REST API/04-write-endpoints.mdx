...
| `POST /api/prepare/subscribe` | 2/min/IP | Prepare subscribing (includes ERC-20 approve when needed) |
| `POST /api/prepare/cancel_subscription` | 2/min/IP | Provider cancel |
| `POST /api/prepare/unsubscribe` | 2/min/IP | Subscriber unsubscribe |
...
| `POST /api/prepare/remit` | 2/min/IP | Prepare permissionless `remit()` |
| `POST /api/transactions/status` | Allowed | Poll confirmation status after broadcast |
