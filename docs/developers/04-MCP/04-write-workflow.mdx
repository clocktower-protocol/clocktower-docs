...
- Counts against the same free/developer [rate limits](/docs/developers/REST%20API/rate-limits) as other MCP tool calls — there is no per-call payment
...
Poll `get_transaction_status` with `{ "txHash": "0x…" }`. Include `requestId` when reporting issues.
