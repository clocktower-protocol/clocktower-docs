...
Prepare and readiness share the **write** rate bucket (RPM + **write daily**). Defaults:
...
| `POST /transactions/status` | Poll confirmation status after broadcast |

See [Rate limits](/docs/developers/REST%20API/rate-limits). For production write volume, use the [SDK](/docs/developers/SDK/installation) with your own RPC.
