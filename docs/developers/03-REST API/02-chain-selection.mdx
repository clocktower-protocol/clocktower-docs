...
sidebar_position: 2
description: Select a protocol chain on REST with optional ?chainId=. MCP and x402 stay on Base.
---

# Chain Selection

REST protocol routes are chain-aware. Pass optional `?chainId=` to read or prepare against a protocol chain. **MCP, x402<IfFeature name="builder">, SIWE, and Builder entitlement</IfFeature> stay on Base** (chainId `8453`).
...
- **MCP** — no `chainId` argument; tools and x402 USDC payments are Base-only
