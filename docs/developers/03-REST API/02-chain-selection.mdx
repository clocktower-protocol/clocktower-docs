...
sidebar_position: 2
description: Select a protocol chain on REST with optional ?chainId=. MCP tools take optional chainId (default Base).
---

# Chain Selection

REST protocol routes are chain-aware. Pass optional `?chainId=` to read or prepare against a protocol chain. MCP protocol tools take optional `chainId` (default Base) — see [MCP chain selection](/docs/developers/MCP/mcp-chain-selection)<IfFeature name="builder">. SIWE and Builder entitlement stay on Base</IfFeature>.
...
MCP protocol tools select a chain with optional `chainId` (default Base). That is independent of REST `?chainId=` and of `DEFAULT_REST_CHAIN_ID`. See [MCP chain selection](/docs/developers/MCP/mcp-chain-selection).

<IfFeature name="builder">
These stay on Base even when REST or MCP selects another protocol chain:

- **SIWE** — challenge/verify use Base chain ID `8453`
- **Builder entitlement** — `BUILDER_SUB_ID` / `BUILDER_SUB_IDS` is always checked on Base

Changing `DEFAULT_REST_CHAIN_ID` does not move SIWE or Builder.
</IfFeature>
