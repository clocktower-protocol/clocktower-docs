...
REST protocol routes are chain-aware. Pass optional `?chainId=` to read or prepare against a protocol chain. **MCP, x402, SIWE, and Builder entitlement stay on Base** (chainId `8453`).
...
Changing `DEFAULT_REST_CHAIN_ID` does not move MCP, SIWE, or Builder.
