...
A prepare call consumes **write RPM**, **write daily**, and **daily total**. `POST /check_subscribe_readiness_by_id` (and the MCP `check_subscribe_readiness_by_id` tool) is **not** in the write bucket (cheap / global RPM only — it does not run on-chain simulation).

MCP `prepare_*` and readiness tools use the same write buckets as the matching REST routes.
