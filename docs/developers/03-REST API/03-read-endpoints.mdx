...
| Endpoint | Description |
|----------|-------------|
| `GET /subscriptions` | Search/discover subscriptions. Query: `provider`, `token`, `frequency`, `cancelled` (default `false`), `includeDetails`, `first` (lane cap: free 10, developer 25), `skip`, plus optional `chainId`. See [Rate limits](/docs/developers/REST%20API/rate-limits) |
...
<td>Subscriptions for an account. Query: <code>bySubscriber=true|false</code> (default <code>false</code> = created as provider; <code>true</code> = subscribed to)</td>
</tr>
<tr>
...
<td>Builder session — your combined subscriber + provider activity</td>
</tr>
</IfFeature>
</tbody>
</table>
