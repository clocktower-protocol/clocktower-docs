...
<IfFeature name="sdk">
SDK `remit()` throws `RemitNotReadyError` and does not send a transaction when the due-queue precheck finds no work. `runRemitUntilCaughtUp` scans once per round, then calls `remit({ skipReadiness: true })`.
</IfFeature>