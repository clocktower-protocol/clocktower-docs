...
On REST, pass the same `?chainId=` on readiness, prepare, and `POST /transactions/status` (`{ "txHash": "0x…" }`). On MCP, pass the same `chainId` on those tools and poll `get_transaction_status` with `txHash`. See [REST chain selection](/docs/developers/REST%20API/chain-selection) and [MCP chain selection](/docs/developers/MCP/mcp-chain-selection).
