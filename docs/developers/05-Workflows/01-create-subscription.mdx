...
On REST, pass `?chainId=` on prepare and on the follow-up `GET /subscriptions/:id` so you stay on the same chain. On MCP, pass the same `chainId` on prepare and `get_transaction_status` (omitted uses Base). See [REST chain selection](/docs/developers/REST%20API/chain-selection) and [MCP chain selection](/docs/developers/MCP/mcp-chain-selection).