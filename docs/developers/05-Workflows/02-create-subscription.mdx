...
3. **Broadcast** — your app sends it to Base mainnet.
...
|------|------|----------|-----|
| 1. Prepare | `POST /api/prepare/create_subscription` | `prepare_create_subscription` | `createSubscription()` |
| 2–3. Sign & broadcast | Your wallet + RPC | Same | `walletClient` via viem |
| 5. Fetch ID | `GET /api/subscriptions/:id` or search | `get_subscription` | `waitForSubscription()` |
