...
|------|------|----------|-----|
| 1. Readiness | `POST /api/check_subscribe_readiness` | `check_subscribe_readiness` | Built into `subscribe()` preflight |
| 2. Prepare | `POST /api/prepare/subscribe` | `prepare_subscribe` | `subscribeWithApproval()` |
| 3–4. Sign & broadcast | Your wallet + RPC | Same | `walletClient` via viem |
| 5. Confirm | `POST /api/transactions/status` | `get_transaction_status` | `waitForReceipt: true` |
...
**Readiness-only** — pass `readinessOnly: true` to `prepare_subscribe` (or `POST /api/prepare/subscribe`) to validate without generating unsigned transactions.