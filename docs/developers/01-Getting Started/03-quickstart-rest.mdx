...
The REST API exposes the same capabilities as MCP over standard HTTP. Access is controlled by free rate limits and optional **developer API keys** (`ctk_…`)<IfFeature name="builder">, plus optional Builder SIWE sessions</IfFeature>. Start with the catalog and a subscription search; you will have real protocol data in under a minute.
...
Protocol routes accept optional `?chainId=` (decimal or CAIP-2, e.g. `8453` or `eip155:8453`). Omitted uses Base (`8453`). Check `GET /catalog` → `chains[]` for which chains are live. MCP tools take optional `chainId` (default Base) — see [MCP chain selection](/docs/developers/MCP/mcp-chain-selection). See [REST chain selection](/docs/developers/REST%20API/chain-selection).
...
Obtain a free `ctk_…` key from [developers.clocktower.finance](https://developers.clocktower.finance). Send:
